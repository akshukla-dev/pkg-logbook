...
if (!empty($this->items))
{
	foreach ($this->items as $watchdog)
	{
		if ($watchdog->params->get('access-edit'))
		{
			$isEditable = true;
			break;
		}
...
<?php if ($this->params->get('filter_field') !== 'hide' || $this->params->get('show_pagination_limit')) : ?>
			...
			<div class="btn-group">
				...
				<?php else : ?>
					<label class="filter-search-lbl element-invisible" for="filter-search">
						<?php echo JText::_('COM_LOGMONITER_' . $this->params->get('filter_field') . '_FILTER_LABEL') . '&#160;'; ?>
					</label>
					<input type="text" name="filter-search" id="filter-search" value="<?php echo $this->escape($this->state->get('list.filter')); ?>" class="inputbox" onchange="document.adminForm.submit();" title="<?php echo JText::_('COM_LOGMONITER_FILTER_SEARCH_DESC'); ?>" placeholder="<?php echo JText::_('COM_LOGMONITER_' . $this->params->get('filter_field') . '_FILTER_LABEL'); ?>" />
				<?php endif; ?>
			</div>
		<?php endif; ?>
	...
	<div class="control-group hide pull-right">
		<div class="controls">
			<button type="submit" name="filter_submit" class="btn btn-primary"><?php echo JText::_('COM_LOGMONITER_FORM_FILTER_SUBMIT'); ?></button>
		</div>
	</div>
...
<?php else : ?>
	<table class="category table table-striped table-bordered table-hover<?php echo $tableClass; ?>">
			...
			<tr>
				...
				<?php if ($this->params->get('list_show_votes', 0) && $this->vote) : ?>
					<th scope="col" id="categorylist_header_votes">
						<?php echo JHtml::_('grid.sort', 'COM_LOGMONITER_VOTES', 'rating_count', $listDirn, $listOrder); ?>
					</th>
				<?php endif; ?>
				<?php if ($this->params->get('list_show_ratings', 0) && $this->vote) : ?>
					<th scope="col" id="categorylist_header_ratings">
						<?php echo JHtml::_('grid.sort', 'COM_LOGMONITER_RATINGS', 'rating', $listDirn, $listOrder); ?>
					</th>
				<?php endif; ?>
				<?php if ($isEditable) : ?>
		...
		<tbody>
		<?php foreach ($this->items as $i => $watchdog) : ?>
			...
			<td headers="categorylist_header_title" class="list-title">
				<?php if (in_array($watchdog->access, $this->user->getAuthorisedViewLevels())) : ?>
					<a href="<?php echo JRoute::_(LogmoniterHelperRoute::getWatchdogRoute($watchdog->slug, $watchdog->catid, $watchdog->language)); ?>">
						<?php echo $this->escape($watchdog->title); ?>
					</a>
					<?php if (JLanguageAssociations::isEnabled() && $this->params->get('show_associations')) : ?>
						<?php $associations = LogmoniterHelperAssociation::displayAssociations($watchdog->id); ?>
				...
				<?php else : ?>
					<?php
					echo $this->escape($watchdog->title) . ' : ';
					$menu   = JFactory::getApplication()->getMenu();
					$active = $menu->getActive();
					$itemId = $active->id;
					$link   = new JUri(JRoute::_('index.php?option=com_users&view=login&Itemid=' . $itemId, false));
					$link->setVar('return', base64_encode(LogmoniterHelperRoute::getWatchdogRoute($watchdog->slug, $watchdog->catid, $watchdog->language)));
					?>
					...
					<?php endif; ?>
				<?php endif; ?>
				<?php if ($watchdog->state == 0) : ?>
					<span class="list-published label label-warning">
								<?php echo JText::_('JUNPUBLISHED'); ?>
							</span>
				<?php endif; ?>
				<?php if (strtotime($watchdog->publish_up) > strtotime(JFactory::getDate())) : ?>
					<span class="list-published label label-warning">
								<?php echo JText::_('JNOTPUBLISHEDYET'); ?>
							</span>
				<?php endif; ?>
				<?php if ((strtotime($watchdog->publish_down) < strtotime(JFactory::getDate())) && $watchdog->publish_down != JFactory::getDbo()->getNullDate()) : ?>
					<span class="list-published label label-warning">
			...
			<?php if ($this->params->get('list_show_author', 1)) : ?>
				<td headers="categorylist_header_author" class="list-author">
					<?php if (!empty($watchdog->author) || !empty($watchdog->created_by_alias)) : ?>
						<?php $author = $watchdog->author ?>
						<?php $author = $watchdog->created_by_alias ?: $author; ?>
						<?php if (!empty($watchdog->contact_link) && $this->params->get('link_author') == true) : ?>
							<?php echo JText::sprintf('COM_LOGMONITER_WRITTEN_BY', JHtml::_('link', $watchdog->contact_link, $author)); ?>
						<?php else : ?>
			...
			<?php if ($this->params->get('list_show_hits', 1)) : ?>
				<td headers="categorylist_header_hits" class="list-hits">
							<span class="badge badge-info">
								<?php echo JText::sprintf('JGLOBAL_HITS_COUNT', $watchdog->hits); ?>
							</span>
						</td>
			<?php endif; ?>
			<?php if ($this->params->get('list_show_logs', 0) && $this->vote) : ?>
				<td headers="categorylist_header_votes" class="list-votes">
					<span class="badge badge-success">
						<?php echo JText::sprintf('COM_LOGMONITER_LOGS_COUNT', $watchdog->log_count); ?>
					</span>
				</td>
			<?php endif; ?>
			<?php if ($isEditable) : ?>
				<td headers="categorylist_header_edit" class="list-edit">
					<?php if ($watchdog->params->get('access-edit')) : ?>
						<?php echo JHtml::_('icon.edit', $watchdog, $watchdog->params); ?>
					<?php endif; ?>
				</td>
			<?php endif; ?>
			</tr>
...
<?php // Code to add a link to submit an watchdog. ?>
