...
require_once JPATH_SITE.'/components/com_logbook/helpers/category.php';

JModelLegacy::addIncludePath(JPATH_SITE.'/components/com_logbook/models', 'LogbookModel');
...
class ModLogbookHelper
{
     ...
     *
     * @return mixed null if no logs based on input parameters else an array containing all the logs
     *
     * @since   1.5
     **/
    public static function getList(&$params)
    {
        // Get an instance of the generic logs model
        $model = JModelLegacy::getInstance('Category', 'LogbookModel', array('ignore_request' => true));
        ...
        // Access filter
        $access = !JComponentHelper::getParams('com_logbook')->get('show_noauth');
        ...
        $case_when1 .= $query->concatenate(array($l_id, 'a.alias'), ':');
        $case_when1 .= ' ELSE ';
        $case_when1 .= $l_id.' END as slug';
        ...
        if ($items) {
            foreach ($items as $item) {
                if ($item->params->get('count_clicks', $params->get('count_clicks')) == 1) {
                    $item->link = JRoute::_('index.php?option=com_logbook&task=log.go&catid='.$item->catslug.'&id='.$item->slug);
                } else {
                    $item->link = $item->url;
                }
            }
